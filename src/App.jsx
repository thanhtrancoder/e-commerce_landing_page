...
import paymentIframeIcons from "./assets/icons/payment_frame_icons.avif";
import verifiedCustomerIcon from "./assets/icons/verified_customer_icon.avif";
  ...
  return (
    <div className="font-[Nunito]">
      ...
      <div className="px-4 py-8">
        ...
        <div>
          <div className="bg-seashell rounded-lg p-3">
            <div>
          ...
          <div className="mt-4 flex items-center gap-2">
            ...
            <p className="text-xs">
              Delivered on{" "}
              <span className="bg-buy-green rounded-sm px-1 py-0.5 text-white">
                Wednesday, 4 February
              </span>{" "}
              with Express Shipping
            </p>
          </div>
          <button className="transition-color font-montserrat mt-3 w-full cursor-pointer rounded-lg bg-black p-3 text-lg font-bold text-white duration-200 hover:opacity-80">
            ADD TO CART
          </button>
          <img src={paymentIframeIcons} className="my-4"></img>
          <div className="space-y-3">
            {buyServices.map((buyService, index) => (
              <BuyService
                key={index}
                icon={buyService.icon}
                text={buyService.text}
              />
            ))}
          </div>
        </div>

        <div className="bg-seashell mt-6 space-y-1.5 rounded-xl p-3">
          <div className="flex items-center gap-4">
            <img src={customerAvatarImg} className="h-10.5 w-10.5"></img>
            <div>
              <div className="flex items-center gap-2">
                <p>Margaret Ellison | FL</p>
                <img
                  src={verifiedCustomerIcon}
                  className="ml-auto h-5 w-auto"
                ></img>
              </div>
              <div className="mt-1.5 flex">
                {[...Array(5)].map((_, i) => (
                  <img key={i} src={starIcon} className="h-4.5 w-auto" />
                ))}
              </div>
            </div>
          </div>
